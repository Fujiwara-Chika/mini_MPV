...
defaults = {
  ...
  audio_types =
  'aac,ac3,aiff,ape,au,dsf,dts,flac,m4a,m4b,mid,midi,mka,mp3,mp4a,oga,ogg,opus,spx,tak,tta,wav,weba,wma,wv',
  image_types =
...
do
  ...
  config.ziggy_path = options.ziggy_pth ~= "default" and mp.command_native({ 'expand-path', options.ziggy_pth }) or
      (join_path(mp.get_script_directory(), join_path('bin', bin)))
end
